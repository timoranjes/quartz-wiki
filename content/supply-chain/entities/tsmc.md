---
title: TSMC
created: 2026-05-29
updated: 2026-06-13
type: entity
tags: [foundry, packaging, capex-cycle]
sources: [raw/articles/informedclearly-semiconductor-bottleneck-2026.md, raw/articles/fusionww-ai-bottleneck-cowos-hbm.md, raw/articles/energy-efficient-compute-is-most-important-attribute-for-cus-796c5d9bec6f0c58.md, raw/articles/tsmc-defends-transistor-scaling-amid-huaweis-hers-law-propos-c0bd77c82eefcf4c.md, raw/articles/tsmc-ceo-cc-wei-says-it-will-be-a-long-time-before-we-can-me-8cfb30fb296f1a33.md]
confidence: high
---

# TSMC (Taiwan Semiconductor Manufacturing Company)

TSMC is the world's largest contract chipmaker and the dominant player in advanced semiconductor manufacturing.

## Market Position

- Controls ~70% of global advanced chip manufacturing
- Manufactures ~85% of advanced AI processors
- Q1 2026: 58% profit jump to NT$572.48 billion ($17.8 billion)
- High-performance computing accounts for 61% of sales

## Key Technologies

- **Advanced nodes:** 3nm and 2nm production, with demand ~3x available capacity
- **CoWoS packaging:** The industry's primary advanced packaging solution for AI accelerators, sold out through 2025 and into 2026
- **HBM demand:** TSMC confirms HBM bit demand growing exponentially, tight supply-demand balance through 2026

## Capacity Expansion

- Investing $165 billion in US expansion: 5 new fabs and 2 packaging facilities in Arizona
- Additional facilities in Germany and Japan
- Combined with significant capex expansion for CoWoS and advanced nodes
- However, even with massive investment, wafer output is still not enough to support AI demand
- **June 2026:** CEO C.C. Wei confirmed at shareholder meeting that it will take "a long time" before TSMC can meet all customer demand from AI hyperscalers. Notably, TSMC is choosing to keep prices stable and refrain from implementing price hikes despite the severe supply-demand imbalance — a strategic decision that may reflect long-term relationship preservation with hyperscaler customers, but also opens opportunity for Intel (18A/14A nodes) and other foundries to capture overflow demand

## Geopolitical Context

- Taiwan concentration creates significant geopolitical risk
- Diversification efforts face lengthy qualification cycles for new facilities
- Industry bifurcating: US/Taiwan/Japan alliance controlling advanced nodes, China building domestic mature node capacity
- **Energy efficiency focus (May 2026):** At Amsterdam Technology Symposium, deputy co-COO Kevin Zhang stated energy-efficient compute is the "most important attribute" for TSMC customers — signaling that power constraints, not just raw performance, are now the primary design driver for AI accelerators
- **Transistor scaling defense (June 2026):** Kevin Zhang reaffirmed that 3D integration is important but transistor scaling remains the semiconductor industry's primary driver of performance and energy-efficiency gains, pushing back against Huawei's "Her's Law" (τ定律) proposal that post-Moore's Law progress would come from architecture and packaging innovation rather than node shrinking

## ITC Import Ban Threat — UMC Patent Dispute (June 2026)

- Five Republican members of Congress urged the US International Trade Commission (ITC) to **block imports of TSMC-manufactured chips** found to infringe US patents
- Patents asserted by **United Microelectronics Corporation (UMC)** — TSMC's domestic foundry rival
- If ITC rules in UMC's favor and imposes an import ban, it would directly disrupt TSMC's ability to ship chips to US customers
- Significance: This is a novel use of patent litigation as a trade weapon against the world's most critical foundry — could create supply chain disruption beyond what export controls achieve
- Political dimension: Republican lawmakers escalating pressure on semiconductor imports, combining IP enforcement with trade policy

## Relationships

- Primary manufacturer for [[nvidia]] GPUs and most AI accelerators
- Key CoWoS packaging provider for the AI industry
- Dependent on [[asml]] EUV lithography equipment for advanced nodes
- Supply chain constraints across [[cowos-packaging]] and [[hbm-memory]]
- Nikon challenging ASML with lower-priced ArF scanners (new immersion platform by 2028) — potential long-term diversification option for litho equipment

## Related

- [[cowos-packaging]]
- [[ai-supply-chain-bottlenecks]]
- [[nvidia]]
- [[hbm-memory]]
