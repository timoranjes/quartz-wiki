...
LlamaIndex agents can function as specialized workers in [[crewai]] setups, where a LlamaIndex-powered agent handles knowledge retrieval and grounding while other crew members focus on different tasks. The framework's tool abstraction also supports [[entities/protocols/mcp-classic|mcp]] integration, allowing LlamaIndex to expose its retrieval and query capabilities as MCP tools.

For multi-agent orchestration, LlamaIndex's workflow system relates to the [[concepts/agent-patterns/orchestrator-workers-classic|orchestrator-workers]] pattern, where a coordinator workflow manages specialized retrieval and synthesis agents. Its ReAct agent implementation follows the [[concepts/agent-patterns/react-pattern-classic|react-pattern]] for tool-using agents that iterate between reasoning and action.
...
Its knowledge graph capabilities connect to broader graph-based reasoning patterns, and its workflow model provides an alternative to the [[concepts/agent-patterns/orchestrator-workers-classic|orchestrator-workers]] architecture for teams that prefer event-driven design over explicit supervisor patterns.

## Related Concepts

[[langchain]], [[openai-agents-sdk]], [[tool-use-pattern]], [[concepts/agent-patterns/react-pattern-classic|react-pattern]], [[concepts/agent-patterns/orchestrator-workers-classic|orchestrator-workers]], [[entities/protocols/mcp-classic|mcp]], [[concepts/agent-patterns/routing-classic|routing]], [[langchain-vs-crewai]]
