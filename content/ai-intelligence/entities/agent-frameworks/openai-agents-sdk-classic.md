...
- **Runs and Traces**: Each agent execution produces a `Run` object with full traceability. You can inspect every tool call, every model response, and the complete reasoning trace. This is critical for debugging and [[concepts/agent-patterns/evaluation-classic|evaluation]].
...
- Contrasts with [[anthropic]]'s approach: Anthropic focuses on protocol standardization ([[entities/protocols/mcp-classic|mcp]]) and product surfaces (Claude Code), while OpenAI focuses on SDK-based developer tooling

## Related Concepts

[[concepts/agent-patterns/function-calling-classic|function-calling]], [[langgraph]], [[llamaindex]], [[coding-agents]], [[langchain]], [[crewai]], [[tool-use-pattern]], [[concepts/agent-patterns/evaluation-classic|evaluation]], [[concepts/agent-patterns/orchestrator-workers-classic|orchestrator-workers]], [[concepts/agent-patterns/routing-classic|routing]]
