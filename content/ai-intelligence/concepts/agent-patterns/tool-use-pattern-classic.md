...
- [[concepts/agent-patterns/react-pattern-classic|react-pattern]] — ReAct is the canonical pattern that structures tool use within
  a reasoning loop (Thought → Action → Observation).
- [[concepts/agent-patterns/planning-pattern-classic|planning-pattern]] — Planning determines which tools to call and in what order;
  tool use is the execution mechanism for plans.
- [[concepts/agent-patterns/reflection-pattern-classic|reflection-pattern]] — Reflection can validate tool outputs (e.g., "did this
  search actually answer my question?") before proceeding.
- [[concepts/agent-patterns/routing-classic|routing]] — Routing decides which tool or agent handles a request; tool use
