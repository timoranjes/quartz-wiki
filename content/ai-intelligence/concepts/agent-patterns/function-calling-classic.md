...
At the protocol level, [[entities/protocols/mcp-classic|mcp]] extends function calling beyond individual models by standardizing how tools are exposed across applications and servers. This means a single MCP server can provide tools to multiple agent frameworks simultaneously, creating a composable tool ecosystem.
...
- You are building any agent that follows the [[tool-use-pattern]] or [[concepts/agent-patterns/react-pattern-classic|react-pattern]], where the model must choose among multiple possible actions
...
- Your tool set is fixed and known in advance, allowing you to use deterministic [[concepts/agent-patterns/routing-classic|routing]] instead of model-based selection
- You are using [[concepts/agent-patterns/prompt-chaining-classic|prompt-chaining]] where each step has a predetermined purpose and no dynamic tool selection is required
...
- Within the **[[concepts/agent-patterns/react-pattern-classic|react-pattern]]**, function calling is the action component of the reasoning-action loop. The model reasons about what to do next, uses function calling to execute the chosen action, observes the result, and repeats.
- Within the **[[concepts/agent-patterns/planning-pattern-classic|planning-pattern]]**, function calls may be sequenced according to a pre-determined plan, with the model using function calling to execute each step and validate progress.

Protocols such as **[[entities/protocols/mcp-classic|mcp]]** extend the same idea outward by standardizing how external capabilities are exposed across applications and servers. This means function calling capability at the model level combines with protocol-level standardization to create a composable tool ecosystem where tools defined once can be used by any compatible agent.
...
[[entities/protocols/mcp-classic|mcp]], [[tool-use-pattern]], [[concepts/agent-patterns/react-pattern-classic|react-pattern]], [[anthropic]], [[coding-agents]], [[langchain]], [[openai-agents-sdk]], [[concepts/agent-patterns/planning-pattern-classic|planning-pattern]], [[concepts/agent-patterns/routing-classic|routing]], [[google-adk]]
