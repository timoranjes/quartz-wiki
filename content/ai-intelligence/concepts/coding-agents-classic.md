...
- **[[concepts/agent-patterns/react-pattern-classic|react-pattern]]** — the most common approach, where the agent alternates between reasoning about the current state and taking actions (reading files, running tests, editing code) in a loop until the task is resolved. This pattern is simple, flexible, and works well for tasks where the path to solution is not known in advance.

- **[[concepts/agent-patterns/planning-pattern-classic|planning-pattern]]** — the agent first creates an implementation plan before executing any changes, which can improve success rates on complex tasks by avoiding blind exploration. The plan may include which files to modify, what order to make changes, and what tests to run at each step.

- **[[concepts/agent-patterns/reflection-pattern-classic|reflection-pattern]]** — the agent reviews its own work after making changes, identifying potential issues before running tests, which reduces wasted compute on obviously broken approaches. This pattern is particularly valuable for catching syntax errors, logical mistakes, and style violations before they reach the test phase.

- **[[concepts/agent-patterns/orchestrator-workers-classic|orchestrator-workers]]** — a supervisor agent decomposes the task and assigns subtasks to specialized worker agents, each handling a specific aspect like file editing, test running, or code review. This pattern scales better for large tasks but adds coordination overhead.

The choice of pattern affects both performance and cost. Simple [[concepts/agent-patterns/react-pattern-classic|react-pattern]] agents work well for localized changes but can get lost in large codebases or enter unproductive loops. Planning-heavy approaches handle complexity better but may over-plan for simple tasks and waste tokens on unnecessary analysis. The best systems often combine patterns dynamically, using planning for complex tasks and direct execution for simple ones.
...
Protocol-level support through [[entities/protocols/mcp-classic|mcp]] is becoming important for coding agents. MCP servers can expose filesystem access, git operations, linting, and build tools as standardized capabilities that any MCP-compatible coding agent can use without custom integration. This means a coding agent built on one framework can access the same tool servers as an agent built on a different framework.
...
Benchmarking coding agents requires more than looking at demos or anecdotal success stories. Systems should be evaluated on real task completion, not just elegant code snippets or cherry-picked examples. That is why benchmarks like [[concepts/evaluation/swe-bench-classic|swe-bench]] matter, and also why they remain incomplete.

Practical [[concepts/agent-patterns/evaluation-classic|evaluation]] should include multiple dimensions:
...
The [[concepts/agent-patterns/evaluation-classic|evaluation]] of coding agents should also consider [[concepts/agent-patterns/function-calling-classic|function-calling]] accuracy, since coding agents rely heavily on invoking file-edit, shell, and test-execution tools correctly. Poor [[concepts/agent-patterns/function-calling-classic|function-calling]] directly translates to broken edits, wasted compute, and developer frustration.

## Ecosystem Connections

Coding agents depend heavily on primitives such as [[concepts/agent-patterns/function-calling-classic|function-calling]] and often operate through variants of [[concepts/agent-patterns/react-pattern-classic|react-pattern]] or related iterative control loops. They also highlight the importance of standardized tool connectivity through systems like [[entities/protocols/mcp-classic|mcp]].

For multi-agent coding scenarios, the [[concepts/agent-patterns/orchestrator-workers-classic|orchestrator-workers]] pattern is increasingly relevant, with a coordinator agent managing specialist agents for different aspects of the development workflow. The [[concepts/agent-patterns/routing-classic|routing]] pattern can also be used to classify incoming tasks and direct them to appropriate specialized agents or human developers based on complexity, domain, or risk level.

The [[concepts/agent-patterns/reflection-pattern-classic|reflection-pattern]] is particularly valuable for coding agents because it allows them to review their own work before committing changes, catching errors that would otherwise require test execution to discover. This reduces wasted compute and improves the quality of intermediate results.
...
The standardization of coding agent capabilities through protocols like [[entities/protocols/mcp-classic|mcp]] and evaluation through benchmarks like [[concepts/evaluation/swe-bench-classic|swe-bench]] suggests the category is maturing from experimental tools to production-ready infrastructure. Teams evaluating coding agents should consider not just current capabilities but the trajectory of improvement and the growing ecosystem of integrations.

## Related Concepts

[[concepts/evaluation/swe-bench-classic|swe-bench]], [[concepts/agent-patterns/function-calling-classic|function-calling]], [[concepts/agent-patterns/react-pattern-classic|react-pattern]], [[entities/protocols/mcp-classic|mcp]], [[tool-use-pattern]], [[concepts/agent-patterns/planning-pattern-classic|planning-pattern]], [[concepts/agent-patterns/evaluation-classic|evaluation]], [[concepts/agent-patterns/orchestrator-workers-classic|orchestrator-workers]], [[concepts/agent-patterns/reflection-pattern-classic|reflection-pattern]], [[concepts/agent-patterns/routing-classic|routing]]
